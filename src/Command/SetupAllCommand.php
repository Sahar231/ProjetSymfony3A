...
#[AsCommand(
    name: 'app:setup-all',
    description: 'Configuration complète: migrations + données',
)]
class SetupAllCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $io->info('📊 Vérification des tables...');
            $schemaManager = $conn->createSchemaManager();
            $tables = $schemaManager->listTableNames();
            
            if (!in_array('quiz', $tables)) {
                $io->error('❌ Table quiz n\'existe pas. Exécutez: php bin/console doctrine:migrations:migrate');
                return Command::FAILURE;
            }
            $io->success('✅ Tables trouvées');

            // Count existing quizzes
            $io->info('📈 Comptage des données existantes...');
            $count = count($this->em->getRepository(Quiz::class)->findAll());
            $io->text("   Quiz actuels: " . $count);

            if ($count > 0) {
                $io->warning('⚠️  Des données existent déjà. Suppression...');
            ...
            // Load new data
            $io->info('📝 Chargement des données de test...');
            ...
            $io->text('   ✓ PHP Basics (5 questions)');
            ...
            $io->text('   ✓ JavaScript ES6 (5 questions)');
            ...
            $io->text('   ✓ SQL Database (5 questions)');
            ...
            $io->text('   ✓ Symfony Framework (5 questions)');

            // Save everything
            $this->em->flush();
            $io->success('✅ 4 quizzes avec 20 questions chargés');

            // Verify
            $finalCount = count($this->em->getRepository(Quiz::class)->findAll());
            $io->success("✅ SETUP COMPLET! $finalCount quizzes dans la base de données");
            
            $io->newLine();
            $io->info('🚀 Prochaines étapes:');
            $io->text('   1. Démarrez le serveur: symfony server:start');
            $io->text('   2. Accédez à http://localhost:8000/student/quiz');
            $io->text('   3. Accédez à http://localhost:8000/admin/quiz');
